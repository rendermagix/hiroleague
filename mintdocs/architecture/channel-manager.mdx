...
**Plugin spawning** — On startup, the Channel Manager reads all enabled `ChannelConfig` entries from `<workspace>/channels/` and spawns one subprocess per channel. Each subprocess receives the local WebSocket address via `--phb-ws` and a log directory via `--log-dir`.
...
graph TD
    ...
    PROC["subprocess.Popen()<br/>channel command + --phb-ws"]
...
| `command` | Subprocess command. Defaults to `["phb-channel-<name>"]` |
