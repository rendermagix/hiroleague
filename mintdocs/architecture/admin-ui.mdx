...
The admin UI runs as a conditional coroutine inside the workspace server process. It starts only when the user passes `--admin` to `hirocli start`.
...
`PORT_OFFSET_ADMIN = 3` and `admin_port_for()` compute the admin port from the workspace slot. The `Config` model carries an `admin_port` field set during `hirocli setup`.
...
| **Set default** | Workspace is not configured | Blocked. Tool raises `WorkspaceError`: run `hirocli setup` first. |
...
Data source: `WorkspaceListTool` (includes `id`, `is_configured`, `gateway_url`, `admin_port`); running state derived from `hirocli.pid` per workspace path.
...
NiceGUI is added to `hirocli/pyproject.toml`:
