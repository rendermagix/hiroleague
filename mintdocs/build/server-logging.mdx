...
description: "How PHB components initialise, configure, and view logs using the shared log_setup module."
---

PHB uses a single logging standard across all components: the `phbcli` server, channel plugin subprocesses, and the gateway. Every process writes its own rotating log file. An optional foreground mode aggregates all logs into one terminal view for development and debugging.
