...
The most common failure is that `phbgateway` is configured to bind to `localhost:8765`.
...
```powershell
uv run phbgateway instance list
uv run phbgateway instance show default
```
...
```powershell
uv run phbgateway stop --instance default
```

Update the registry host to `0.0.0.0`:

```powershell
$reg = "$env:LOCALAPPDATA\phbgateway\registry.json"
...
```powershell
uv run phbgateway start --instance default --foreground
```
...
```text
%LOCALAPPDATA%\phbgateway\registry.json
```
...
```json
{
  ...
  "instances": {
    "default": {
      "name": "default",
      "path": "C:\\Users\\augr\\AppData\\Local\\phbgateway\\instances\\default",
      "host": "0.0.0.0",
      "port": 8765
    }
  }
...
## Checking phbcli workspace defaults

For a local setup, no changes are needed.

`phbcli` runs on the same machine as the gateway, so its default workspace config can still use:
...
- **Desktop `phbcli`:** `ws://localhost:8765`
- **Physical phone app:** `ws://<LAN_IP>:8765`

Only change `phbcli` workspace `gateway_url` if the gateway is moved to a different machine.
