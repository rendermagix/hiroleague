...
hirocli workspace create <name> [--path P] [--set-default]
hirocli workspace remove <name-or-id> [--purge] [--yes]
hirocli workspace update <name-or-id> [--name N] [--set-default] [--gateway-url URL]
...
def register(workspace_app: typer.Typer, console: Console) -> None:
    ...
    def workspace_list() -> None:
        ...
        if not result.workspaces:
            console.print(
                ...
                "Run [bold]hirocli workspace create <name>[/bold] to get started.[/dim]"
            )
            return
        ...
        for ws in result.workspaces:
            ...
            pid = read_pid(workspace_path, "hirocli.pid")
    ...
    def workspace_create(
        ...
        console.print(f"\nNext: [bold]hirocli setup --workspace {result.name}[/bold]")
    ...
    def workspace_update(
        ...
        gateway_url: Optional[str] = typer.Option(
            ...
            help="New gateway WebSocket URL (light update — no key regen). "
                 "For full reconfiguration use 'hirocli setup'.",
        ),
    ) -> None:
