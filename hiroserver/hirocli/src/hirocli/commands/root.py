...
def register(app: typer.Typer, console: Console) -> None:
    ...
    def setup(
        ...
        if result.server_started:
            ...
            console.print(
                f"\nRun [bold]hirocli start --workspace {result.workspace}[/bold] to start the server."
            )
    ...
    def uninstall(
        ...
        console.print(
            "\n[bold]To fully remove hirocli, run one of:[/bold]\n"
            "  [cyan]uv tool uninstall hirocli[/cyan]       (if installed via uv tool)\n"
            "  [cyan]pip uninstall hirocli[/cyan]            (if installed via pip)\n"
        )
...
def _print_workspace_status_entry(
    ...
    title = f"hirocli status — {ws.name}"
    if ws.is_default:
